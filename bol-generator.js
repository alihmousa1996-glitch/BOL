    ...
    <div class="vehicle-row">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" class="form-input vehicle-wo" placeholder="WR228154">
        </div>
        <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
        <div>
          ...
          <input type="text" class="form-input vehicle-year" placeholder="2021">
        </div>
        <div>
          <label class="form-label">Make</label>
          <input type="text" class="form-input vehicle-make" placeholder="NISSAN">
        </div>
        <div>
          <label class="form-label">Model</label>
          <input type="text" class="form-input vehicle-model" placeholder="ROGUE PLATINUM">
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
        ...
        <div>
          <label class="form-label">Measurement (ft³)</label>
          <input type="number" step="0.01" class="form-input vehicle-measurement" placeholder="509.88">
        </div>
      </div>
    </div>
  `;
...
function loadSample() {
  ...
  document.getElementById('blNumber').value = 'EBKG15426672';
  
  // Routing
  document.getElementById('forwardingAgent').value = 'CIF Canada';
  document.getElementById('agentAddress').value = '1500, Marcel-Benoit, Laval, QC H7C 0A9';
  ...
  document.getElementById('cersNumber').value = 'WI4066202601134969629';
  document.getElementById('freeDays').value = '21 FREE DAYS';
  ...
  vehicles.forEach((v, i) => {
    ...
    row.querySelector('.vehicle-wo').value = v.wo;
    row.querySelector('.vehicle-vin').value = v.vin;
    row.querySelector('.vehicle-year').value = v.year;
    row.querySelector('.vehicle-make').value = v.make;
    row.querySelector('.vehicle-model').value = v.model;
    row.querySelector('.vehicle-weight').value = v.weight;
    row.querySelector('.vehicle-measurement').value = v.measurement;
  });
...
function generatePDF() {
  ...
  const data = {
    ...
    exportRef: document.getElementById('exportRef').value,
    forwardingAgent: document.getElementById('forwardingAgent').value,
    ...
    cersNumber: document.getElementById('cersNumber').value,
  ...
  vehicleRows.forEach(row => {
    vehicles.push({
      wo: row.querySelector('.vehicle-wo').value,
      ...
      measurement: parseFloat(row.querySelector('.vehicle-measurement').value) || 0
    });
  });
  
  // Calculate totals
  const totalWeight = vehicles.reduce((sum, v) => sum + v.weight, 0);
  const totalMeasurement = vehicles.reduce((sum, v) => sum + v.measurement, 0);
  const totalWeightLbs = (totalWeight * 2.20462).toFixed(2);
  const totalMeasurementVlb = (totalMeasurement * 1.04).toFixed(2);
  ...
  doc.setFont(undefined, 'bold');
  doc.text('5a. B/L NUMBER', 110, yPos);
  yPos += 4;
  doc.setFont(undefined, 'normal');
  doc.text(data.blNumber, 110, yPos);
  ...
  doc.setFont(undefined, 'bold');
  doc.text('6. EXPORT REFERENCES', 110, yPos);
  yPos += 4;
  doc.setFont(undefined, 'normal');
  doc.text(data.exportRef, 110, yPos);
  ...
  doc.text(`${totalMeasurement.toFixed(2)} ft³`, 180, yPos);
  
  yPos += 4;
  doc.text(`SEAL: ${data.sealNumber}`, 10, yPos);
  doc.text(`"${totalWeightLbs} Lb"`, 150, yPos);
  doc.text(`"${totalMeasurementVlb} Vlb"`, 180, yPos);
  ...
  const vehicleTableData = vehicles.map((v, i) => {
    ...
    const measurementVlb = (v.measurement * 1.04).toFixed(2);
    ...
    return [
      ...
      `${v.measurement.toFixed(2)} ft³\n"${measurementVlb} Vlb"`
    ];
  });
  
  doc.autoTable({
    startY: yPos,
    head: [['MARKS', 'QTY', 'DESCRIPTION OF COMMODITIES', 'GROSS WEIGHT', 'MEASUREMENT']],
  ...
  // Additional Info
  if (data.cersNumber) {
    doc.text(`CERS# ${data.cersNumber}`, 10, yPos);
    yPos += 5;
  }
  
  if (data.freeDays) {
    doc.text(data.freeDays, 10, yPos);
    yPos += 5;
  }
  ...
  yPos += 5;
  doc.text(`B/L No. ${data.blNumber}`, 150, yPos);
...
IN WITNESS WHEREOF three (3) original Bills of Lading have been signed, not otherwise stated above, one of which being accomplished the others shall be void.`;
  ...
  // Save PDF
  const filename = `BOL_${data.blNumber || data.docNumber}_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
